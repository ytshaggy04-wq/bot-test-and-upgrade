...
case 'remind': {
    ...
    // Usage: .schedule 94768069800|10s|Hello Message  හෝ  group JID එකක්
    const input = args.join(' ');
    const parts = input.split('|');

    if (parts.length < 3) {
        let helpText = `🎀 *𝗦𝗖𝗛𝗘𝗗𝗨𝗟𝗘𝗥  𝗠𝗔𝗡𝗔𝗚𝗘𝗥*\n\n` +
            `📝 *𝖴𝗌𝖺𝗀𝖾 :* \`.schedule TARGET | TIME | MESSAGE\`\n` +
            `✨ *𝖤𝗑𝖺𝗆𝗉𝗅𝖾 :* \`.schedule 94768069800 | 10s | Hello Bro\`\n` +
            `🫧 *𝖦𝗋𝗈𝗎𝗉 𝖤𝗑 :* \`.schedule 1203630...g.us | 5m | Meeting start!\`\n\n` +
    ...
    let timeArg = parts[1].trim();
    let reminderMsg = parts[2].trim();

    // Number එකක් නම් JID එකකට හරවා ගැනීම (94768... -> 94768...@s.whatsapp.net)
    ...
    // නියමිත වෙලාව ආවම අදාළ target එකට මැසේජ් එක යැවීම
    setTimeout(async () => {
        try {
